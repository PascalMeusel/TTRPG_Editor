...
class MapView:
    ...
    def draw_viewer_canvas(self, map_model, controller):
        """Draws the entire live scene on the viewer canvas, including background, tokens, and overlays."""
        self.viewer_canvas.delete("all")
        if not map_model.name: return
        self._display_map_image(map_model.name)
        grid_size = map_model.grid_size
    ...
    def _draw_token(self, canvas, token_data, grid_size, preview_pos=None):
        """Helper to draw a single token. Uses pixel-based preview_pos if provided for smooth dragging."""
        ...
        canvas.create_oval(center_x - radius, center_y - radius, center_x + radius, center_y + radius, fill=color, outline="white", width=1)
        canvas.create_text(center_x, center_y, text=token_data['name'][0], fill="white", font=("Arial", 10, "bold"))
    ...
    def _draw_selection_overlay(self, map_model, controller):
        ...
        if len(controller.selected_tokens) == 1:
            ...
            if move_dist > 0:
                ...
                self.viewer_canvas.create_oval(center_x - radius, center_y - radius, center_x + radius, center_y + radius, outline="yellow", dash=(4, 4), width=2)
            ...
            self.viewer_canvas.create_line(x1, y1, x2, y2, fill="yellow", width=2, dash=(4, 4))
    ...
    def _display_map_image(self, map_name):
